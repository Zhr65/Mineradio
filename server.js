...
async function handleSongUrl(id, loginInfo, qualityPreference) {
  ...
  if (trialFallback) {
    // TuneHub 兜底：VIP试听时尝试拿完整链接
    try {
      const thResp = await fetch(TUNEHUB_BASE + '/v1/parse', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': TUNEHUB_KEY },
        body: JSON.stringify({ platform: 'netease', ids: String(id), quality: '320k' })
      });
      const thData = await thResp.json();
      const thSong = thData && thData.data && thData.data.data && thData.data.data[0];
      if (thSong && thSong.url) {
        console.log('[SongUrl] TuneHub ->', thSong.actualQuality || 'ok');
        return { url: thSong.url, trial: false, playable: true, level: thSong.actualQuality || 'standard', quality: thSong.actualQuality || 'standard', br: 320000, requestedQuality, source: 'tunehub' };
      }
    } catch (e) { console.log('[SongUrl] TuneHub failed:', e.message); }
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/playlist/unsubscribe') {
    try {
      const info = await requireLogin(res);
      if (!info) return;
      const pid = Number(url.searchParams.get('id') || '');
      if (!pid) { sendJSON(res, { error: 'Missing playlist id' }, 400); return; }
      let r1 = await playlist_subscribe({ id: pid, t: 0, cookie: userCookie, timestamp: Date.now() });
      let code = (r1.body && r1.body.code) || r1.code || 0;
      if (code === 200) { sendJSON(res, { loggedIn: true, id: pid, unsubscribed: true }); return; }
      let r2 = await playlist_delete({ id: pid, cookie: userCookie, timestamp: Date.now() });
      code = (r2.body && r2.body.code) || r2.code || 0;
      if (code === 200) { sendJSON(res, { loggedIn: true, id: pid, unsubscribed: true }); }
      else { sendJSON(res, { error: 'failed (' + code + ')', unsubscribed: false }, 500); }
    } catch (err) { console.error('[PlaylistUnsubscribe]', err); sendJSON(res, { error: err.message }, 500); }
    return;
  }
  // ---------- 从歌单移除歌曲 ----------
